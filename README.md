...
graph TD
    ...
    B --> E[Роль рекуррентных связей]
    B --> F[Предиктивное кодирование]
    B --> G[Внимание и рабочая память]

    C --> H[Сознание как эмерджентное свойство]
    C --> I[Примеры эмерджентности]
    
    D --> J[Функциональные связи]
    D --> K[Глобальная рабочая площадка]
    D --> L[Метрика Φ]

    E --> M[Рекуррентные контуры]
    E --> N[Механизмы рекуррентной активации]
    E --> O[Нарушения рекуррентности]

    F --> P[Основные принципы]
    F --> Q[Иерархическая структура]
    F --> R[Принцип свободной энергии]

    G --> S[Механизмы отбора]
    G --> T[Кратковременное удержание информации]
```
