...
```mermaid
flowchart TD
    A[Архитектура системы восприятия] --> B[Иерархическая организация сенсорных систем]
    A --> C[Нейронные сети восприятия]
    
    B --> B1[Первичные сенсорные зоны]
    B1 --> B11["Зрительная кора (V1): Обработка ориентации, контраста, движения"]
    B1 --> B12["Слуховая кора (A1): Анализ частоты, интенсивности, структуры звуков"]
    B1 --> B13["Соматосенсорная кора (S1): Давление, вибрация, температура"]
    
    B --> B2[Вторичные ассоциативные области]
    B2 --> B21[Вентральный поток: Распознавание объектов, формирование семантики]
    B2 --> B22[Дорсальный поток: Положение, движение, действия]
    B2 --> B23[Вторичные слуховые и соматосенсорные области: Интеграция сложных признаков]
    
    B --> B3[Мультимодальные интегративные центры]
    B3 --> B31[Теменная кора: Пространственные репрезентации]
    B3 --> B32[Префронтальная кора: Внимание, решения, осознанный опыт]
    B3 --> B33[Медиальные структуры: Контекст, эпизодическая память]

    C --> C1[Вентральный и дорсальный потоки обработки]
    C1 --> C11["Вентральный поток (что): Идентификация объектов, височные области"]
    C1 --> C12["Дорсальный поток (где/как): Локализация объектов, контроль действий"]

    C --> C2[Фронто-париетальная сеть интеграции]
    C2 --> C21[Интеграция вентрального и дорсального потоков]
    C2 --> C22[Формирование осознанного восприятия, контроль внимания]

    C --> C3[Таламокортикальные петли обратной связи]
...
flowchart TD
    ...
    A --> C[Пространственная интеграция]

    B --> B1["Гамма-осцилляции (30-100 Гц)"]
    B1 --> B11[Связывание признаков: форма, цвет, движение]
    B1 --> B12[Нарушения: дефициты восприятия и когнитивных функций]
    B --> B2["Тета-ритм (4-8 Гц)"]
    B2 --> B21[Контекстуальная информация и пространственно-временные отношения]
    B2 --> B22[Связь сенсорных, мнемонических и исполнительных процессов]
    B --> B3["Альфа-активность (8-12 Гц)"]
    B3 --> B31[Торможение и подавление нерелевантной информации]
    B3 --> B32[Селективное внимание и фильтрация сигналов]

    C --> C1[Конвергенция сенсорных путей]
    C1 --> C11[Обработка на высоких уровнях иерархии]
    C1 --> C12[Формирование сложных рецептивных полей]
    C --> C2[Мультисенсорные нейроны]
    C2 --> C21[Интеграция информации из разных модальностей]
    C2 --> C22[Когерентные мультимодальные представления]
    C --> C3[Топографическая организация связей]
...
```mermaid
flowchart TD
    A[Предиктивное кодирование] --> B[Байесовские вычисления в мозге]
    A --> C[Активное восприятие]

    B --> B1[Генерация предсказаний]
    B1 --> B11[Верхние уровни генерируют прогнозы]
    B1 --> B12[Модели основаны на опыте]
    B --> B2[Вычисление ошибок предсказания]
    B2 --> B21[Сравнение входов и предсказаний]
    B2 --> B22[Ошибки передаются вверх по иерархии]
    B --> B3[Обновление внутренних моделей]
    B3 --> B31[Адаптация на основе ошибок]
    B3 --> B32[Интеграция новых данных в модели]

    C --> C1[Саккадические движения глаз]
    C1 --> C11[Перемещение фокуса на объекты]
    C1 --> C12[Синхронизация с предиктивным кодированием]
    C --> C2[Тактильное исследование]
    C2 --> C21[Активные движения для исследования объектов]
    C2 --> C22[Точные и детальные восприятия]
    C --> C3[Моторные компоненты восприятия]
...
flowchart TD
    ...
    A --> C[Электрофизиологические записи]

    B --> B1[fMRI-данные]
    B1 --> B11[Активация первичных сенсорных областей]
    B1 --> B12[Иерархическая обработка сложных объектов]
    B --> B2[MEG-исследования]
    B2 --> B21[Временная динамика обработки]
    B2 --> B22[Рекуррентные взаимодействия]
    B --> B3[Коннектомные исследования]
    B3 --> B31[Анатомические связи для интеграции]
    B3 --> B32[Рекуррентные контуры и топология]

    C --> C1[Одиночные нейроны]
    C1 --> C11[Селективность к признакам стимулов]
    C1 --> C12[Интеграция сложных комбинаций]
    C --> C2[Локальные полевые потенциалы]
    C2 --> C21[Гамма-ритм связывает признаки объектов]
    C2 --> C22[Тета-ритм кодирует контекст]
    C2 --> C23[Альфа-ритм отражает торможение]
    C --> C3[Корковые осцилляции]
    C3 --> C31[Гамма-активность при сознательном восприятии]
...
```mermaid
flowchart TD
    A[Математическое моделирование] --> B[Динамические системы]
    B --> B1[Состояние системы]
    B --> B2[Входы]
    B --> B3[Параметры]
    B --> B4[Динамическое уравнение]
    B1 --> B1a[Активность нейронов]
    B1 --> B1b[Синаптические связи]
    B1 --> B1c[Уровни нейромедиаторов]
    B2 --> B2a[Сенсорные стимулы]
    B2 --> B2b[Обратные связи]
    B2 --> B2c[Внимание и намерения]
    B3 --> B3a[Структура сетей]
    B3 --> B3b[Временные константы]
    B3 --> B3c[Правила пластичности]
    B4 --> B4a[Устойчивость и бифуркации]
    B4 --> B4b[Влияние параметров]
    B4 --> B4c[Механизмы адаптации]
    A --> C[Информационно-теоретические метрики]
    C --> C1[Взаимная информация]
    C --> C2[Каузальная плотность]
    C --> C3[Интегративная сложность]
    C1 --> C1a[Степень зависимости между уровнями]
    C1 --> C1b[Предсказание активности]
    C2 --> C2a[Рекуррентные связи]
    C2 --> C2b[Взаимная причинность]
