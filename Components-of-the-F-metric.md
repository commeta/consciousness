...
```mermaid
flowchart TD
    Φₑ --> C_t["C_t: Когнитивная сложность"]
    Φₑ --> S_t["S_t: Социальный контекст"]
    Φₑ --> A_t["A_t: Адаптивность"]
    Φₑ --> M_t["M_t: Мотивация"]
    Φₑ --> P_t["P_t: Физиологические параметры"]
    Φₑ --> V_t["V_t: Вариативность поведения"]
    Φₑ --> T_t["T_t: Темпоральные характеристики"]
    Φₑ --> K_t["K_t: Контекстуальность"]
    Φₑ --> I["I: Интегративность"]
    Φₑ --> C["C: Когерентность"]
    Φₑ --> F["F: Гибкость"]
    Φₑ --> M["M: Метакогнитивная осведомлённость"]
    Φₑ --> I_s["I_s: Социальная интерактивность"]
    Φₑ --> E_eff["E_eff: Энергетическая эффективность"]
    Φₑ --> I_c["I_c: Информационная ёмкость"]
    Φₑ --> S_st["S_st: Стабильность"]
    Φₑ --> D_s["D_s: Семантическая глубина"]
    Φₑ --> Q_c["Q_c: Квантовая когерентность"]
    Φₑ --> E_c["E_c: Эмерджентная сложность"]
    Φₑ --> N_p["N_p: Нейропластичность"]
    Φₑ --> I_i["I_i: Информационная интеграция"]
    Φₑ --> T_i["T_i: Темпоральная интеграция"]
    Φₑ --> E_i["E_i: Эмоциональный интеллект"]
    Φₑ --> S_o["S_o: Самоорганизация"]
    Φₑ --> I_conn["I_conn: Информационная связность"]

    %% Связи между компонентами
    C_t --> I
    C_t --> C
    S_t --> R["Рекуррентность"]
    M_t --> I
    M_t --> R
    P_t --> E_eff
    V_t --> F
