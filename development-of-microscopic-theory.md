...
```mermaid
graph TD
    A[Разработка микроскопической теории] --> B[Интеграция квантовой информации в пространстве-времени]
    B --> C[Рекуррентные взаимодействия]
    C --> D[Макроскопические проявления: тёмная материя и энергия]
    C --> E[Математическая формализация]
    E --> F[Теория категорий]
    E --> G[Топология и алгебраическая геометрия]
    E --> H[Нетрадиционные геометрии и фрактальные структуры]
    F --> F1[Категории: состояния квантовой информации]
    F --> F2[Функторы: преобразования состояний]
    G --> G1[Топологические инварианты: фундаментальная группа, гомологии]
    G --> G2[Алгебраическая геометрия: решения полиномиальных уравнений]
    H --> H1[Фрактальная геометрия]
    H --> H2[Самоподобные структуры]
    C --> I[Связь с квантовой гравитацией]
    I --> J[Петлевая квантовая гравитация]
    I --> K[Теория струн]
    I --> L[Квантовая информация и гравитация]
    J --> J1[Спиновые сети: квантование пространства-времени]
    J --> J2[Петлевые структуры и интеграция информации]
    K --> K1[Одномерные струны и дополнительные измерения]
    K --> K2[Принцип голографии и AdS/CFT корреспонденция]
    L --> L1[ER=EPR: связь запутанности и геометрии]
    L --> L2[Информация влияет на структуру пространства-времени]
```
...
```mermaid
graph TD
    A[Введение новых физических величин и параметров] --> B["Плотность интегрированной квантовой информации (ρ_(IQI))"]
    A --> C["Параметр рекуррентности (R)"]
    A --> D[Эффективное уравнение состояния тёмной энергии]
    A --> E[Модифицированные уравнения Эйнштейна]
    A --> F[Интерпретация и последствия]
    
    B --> B1[Определение: лимит прироста информации к объёму]
    B --> B2[Связь с локальной плотностью квантовых корреляций]
    B --> B3[Выражение через энтропию фон Неймана]
    
    C --> C1[Определение: вероятность возврата системы]
    C --> C2[Отражение циклических процессов]
    C --> C3[Связь с топологией и числами Бетти]
    
    D --> D1["Обобщённое уравнение состояния: w_(eff)"]
    D --> D2["Функция f(ρ_(IQI), R)"]
    D --> D3[Физические сценарии: ускоренное расширение Вселенной]
    
    E --> E1["Дополнительный тензор энергии-импульса (T_(μν)^((IQI)))"]
    E --> E2[Структура тензора: плотность, давление, анизотропия]
    E --> E3[Уравнения сохранения и модели зависимости]
    
    F --> F1[Эмерджентность гравитации из квантовой информации]
    F --> F2[Объяснение тёмной материи и энергии]
    F --> F3[Связь с наблюдениями и ограничениями параметров]
```
