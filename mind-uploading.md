...
Фотометрия по оптическим волокнам позволяет следить за динамикой популяционных флуоресцентных сигналов у свободно движущихся животных, однако усредняет данные по большому объёму ткани и не даёт локализации отдельных клеток ([Научный Директор][75], [PubMed][76]).
