...
```mermaid
flowchart TD
    A[Введение] --> B["Плотность интегрированной квантовой информации (ИКИ)"]
    B --> C["Тензор энергии-импульса T(μν)^(ИКИ)"]
    C --> D["Форма T(μν)^(ИКИ)"]
    D --> E["Градиенты ρ(ИКИ) и потенциальная энергия V(ρ(ИКИ))"]
    E --> F["Уравнения поля: G(μν) + Λg(μν) = 8πG(T(μν)^(материя) + T(μν)^(ИКИ))"]
    F --> G["Уравнение движения для ρ(ИКИ)"]
    G --> H["Связь V(ρ(ИКИ)) с кривизной пространства-времени R"]
    H --> I["Динамика: □ρ(ИКИ) - m²ρ(ИКИ) - ξRρ(ИКИ) = 0"]
    I --> J[Взаимовлияние ИКИ и геометрии]
    J --> K[Космологические применения]
    K --> L[Инфляция и темная энергия]
    J --> M[Применение к черным дырам]
    M --> N[Изменение структуры горизонта событий]
    M --> O[Регуляризация сингулярностей]
    K --> P["Анализ уравнений для ρ(ИКИ) в космосе"]
    P --> Q[Численная оценка для κ, m, ξ]
    Q --> R["Пустое пространство: свободное распространение ρ(ИКИ)"]
    Q --> S[Присутствие материи: взаимодействие через R]
    A --> T[Вызовы и дальнейшие направления]
    T --> U[Экспериментальная проверка]
    U --> V[Отклонения в движении небесных тел]
    U --> W[Аномалии в космическом микроволновом фоне]
    T --> X[Квантовая гравитация]
    X --> Y[Интеграция с теориями ПКГ и струн]
    T --> Z[Математическая строгость]
    Z --> AA[Устойчивость решений]
    Z --> AB[Связь с ОТО и квантовой теорией поля]
    Z --> AC[Численные методы анализа]
    T --> AD[Перспективы развития]
```
