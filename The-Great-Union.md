...
```mermaid
graph TD
    A[Тёмная материя и энергия как эмерджентные явления] --> B[Эмерджентность и квантовая информация]
    B --> C[Рекуррентное пространство-время]
    C --> D[Топология и геометрия рекуррентности]
    B --> E[Интеграция информации в космологии]
    E --> F[Новые физические параметры]
    F --> G[Плотность интегрированной квантовой информации ρ_IQI]
    F --> H[Параметр рекуррентности R]
    G --> I[Модификация уравнений Эйнштейна]
    H --> I
    I --> J[Эффективное уравнение состояния тёмной энергии]
    J --> K[Объяснение ускоренной экспансии Вселенной]
    D --> L[Объединение квантовой теории поля и гравитации]
    L --> M[Интеграция в Теорию Всего]
    
    subgraph "Эмерджентные процессы"
        A
        B
        C
        E
    ...
    subgraph "Теоретическое объединение"
        D
        L
        M
    end
