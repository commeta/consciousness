...
Согласно теории Эмергентной Интеграции и Рекуррентного Отображения (ЭИРО), ключевым параметром, определяющим уровень сознательного восприятия, является эмергентная интегрированная информация (Φₑ). Этот параметр можно использовать для количественной оценки процессов, лежащих в основе сновидений.

Расчет Φₑ во время сна

Φₑ определяется следующим образом:
...
Ключевые аспекты расчета Φₑ во время сна:

1. Интегрированная информация I(x,t): Отражает степень объединения информации в данной точке пространства-времени мозга во время различных фаз сна. Она зависит от структуры и динамики нейронных связей.

2. Рекуррентная активность R(x,t): Характеризует интенсивность рекуррентных взаимодействий в конкретной области мозга в момент времени t. Рекуррентность определяет способность к динамической интеграции информации во время сна.

3. Веса синаптических связей wᵢ: Модулируют вклад различных нейронных ансамблей в общую интегрированную информацию Φₑ. Эти веса могут изменяться в зависимости от фазы сна.

4. Пространственное интегрирование ∫dx: Суммирование по всем нейронным ансамблям и интегрирование по пространству мозга дает интегральную меру эмергентной интегрированной информации Φₑ(t) в конкретный момент времени t во время сна.
...
- W_hh, W_hx - матрицы весов рекуррентных и входных связей
- b_h - вектор смещений скрытого слоя
- φ - функция активации (например, tanh)
...
- P(s_t | θ_t) - вероятность наблюдать сенсорный сигнал s_t при параметрах модели θ_t.
- N(s_t; ^s_t, Σ_t) - нормальное распределение с математическим ожиданием ^s_t (предсказанным сигналом) и ковариацией Σ_t.

Обновление параметров по правилу Байеса

Согласно правилу Байеса, апостериорная вероятность параметров модели θ_t обновляется следующим образом:

`P(θ_t | s_t) ∝ P(s_t | θ_t) P(θ_t)`

Где:

- P(θ_t | s_t) - апостериорная вероятность параметров после наблюдения данных s_t.
- P(s_t | θ_t) - правдоподобие данных при заданных параметрах.
- P(θ_t) - априорное распределение параметров.
...
Таким образом, мозг стремится обновлять свои внутренние модели θ_t таким образом, чтобы минимизировать ошибки предсказания s_t - ^s_t на всех уровнях иерархии.
