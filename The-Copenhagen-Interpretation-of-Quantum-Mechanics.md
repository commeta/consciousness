...
```mermaid
graph TD
    A[Копенгагенская интерпретация] --> B[Волновая функция]
    B --> C[Коллапс при измерении]
    A --> D[Роль наблюдателя]
    D --> E[Внешний классический наблюдатель]
    E --> F[Неясность в определении]

    A --> G[Теория ЭИРО]
    G --> H[Эмергентная информация Ψ]
    G --> I[Рекуррентность R]
    H --> J[Интеграция информации в системе]
    I --> K[Динамическая обработка]
    J --> L[Метрические свойства]
    L --> M[Дефиниция ρᵢₓᵢ]
    K --> N[Квантовые взаимодействия]

    C --> O[Пересмотр через ЭИРО]
    O --> P[Модификация уравнений Шрёдингера]
    P --> Q[Оператор H]
    O --> R[Процесс измерения как закономерность]

    O --> S[Космологические последствия]
    S --> T[Объяснение темной энергии]
    T --> U[Модификация уравнений Эйнштейна]
    S --> V[Уравнение состояния wₑₓ]
    S --> W[Ускоренное расширение]

    O --> X[Интеграция в квантовую механику]
    X --> Y[Параметры Ψ и R]
    X --> Z[Коллапс как эмергентное явление]

    Z --> AA[Устранение внешнего наблюдателя]
    Y --> AB[Связь с пространством-временем]
    AB --> AC[Тензор энергии-импульса]
```
