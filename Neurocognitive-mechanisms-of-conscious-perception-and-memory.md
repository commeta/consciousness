...
```mermaid
flowchart TD
    A[Архитектура памяти] --> B[Сенсорная память]
    A --> C[Рабочая память]
    A --> D[Долговременная память]

    B --> E["Иконическая (зрительная)"]
    E --> F[Длительность: 100-500 мс]
    E --> G[Объем: до 12-16 элементов]
    E --> H["Нейронный субстрат: первичная зрительная кора (V1)"]
    E --> I[Механизм: послеобразы и персистенция нейронной активности]

    B --> J["Эхоическая (слуховая)"]
    J --> K[Длительность: 2-4 секунды]
    J --> L[Объем: 4-5 звуковых паттернов]
    J --> M[Нейронный субстрат: верхняя височная извилина]
    J --> N[Механизм: временная интеграция слуховых стимулов]

    C --> O[Центральный исполнитель]
    O --> P[Локализация: префронтальная кора]
    O --> Q[Функции]
    Q --> R[Распределение ресурсов внимания]
    Q --> S[Координация подсистем]
    Q --> T[Переключение между задачами]
    O --> U[Пропускная способность: 1-2 задачи]

    C --> V[Фонологическая петля]
    V --> W[Компоненты]
    W --> X["Фонологическое хранилище (1.5-2 сек)"]
    W --> Y[Артикуляторный процесс повторения]
    V --> Z[Нейронный субстрат: зона Брока, теменная кора]
    V --> AA[Объем: 7±2 элемента]

    C --> AB[Визуально-пространственный блокнот]
    AB --> AC[Подсистемы]
    AC --> AD["Визуальный кэш (форма, цвет)"]
    AC --> AE["Пространственный журнал (локация, движение)"]
    AB --> AF[Локализация: теменно-затылочные области]
    AB --> AG[Объем: 3-4 объекта]

    C --> AH[Эпизодический буфер]
    AH --> AI[Функция: интеграция информации из разных источников]
    AH --> AJ["Объем: 4 эпизода/chunk'а"]
    AH --> AK[Нейронный субстрат: медиальная височная доля]

    D --> AL[Декларативная память]
    AL --> AM[Эпизодическая]
    AM --> AN[Локализация: гиппокамп, префронтальная кора]
    AM --> AO[Механизм: долговременная потенциация]
    AM --> AP["Консолидация: системная (дни-недели)"]

    AL --> AQ[Семантическая]
    AQ --> AR[Локализация: височная кора]
    AQ --> AS[Организация: семантические сети]
    AQ --> AT[Объем: практически неограничен]

    D --> AU[Процедурная память]
    AU --> AV[Локализация: базальные ганглии, мозжечок]
...
graph TD
    ...
    B --> C["Эмергентная интегрированная информация Φₑ(t)"]
    C --> D["Формула: Σ wᵢ ∫ I(x, t) R(x, t) dx"]
    D --> E[Весовые коэффициенты wᵢ]
    D --> F["Интеграция I(x, t)"]
    D --> G["Рекуррентная активность R(x, t)"]
    F --> H[Структура и динамика нейронных связей]
    G --> I[Интенсивность взаимодействий]
    C --> J["Интегральная мера Φₑ(t)"]
    J --> K[Уровень сознательного восприятия]
    
    A --> L[Экспериментальные подтверждения]
    L --> M[Усиление рекуррентной активности]
...
```mermaid
flowchart TD
    A[Механизмы памяти в контексте ЭИРО] --> B[Консолидация памяти]
    B --> C[Начальное экспоненциальное затухание]
    C --> D[Константа затухания τ]
    B --> E[Активный процесс консолидации]
    E --> F[Интеграл по времени s]
    E --> G[Эмергентная интеграция информации Φₑ]
    B --> H["Ядро консолидации K(s)"]
    H --> I[Форма определяется механизмами]

    A --> J[Нейропластичность]
    J --> K["Долговременная потенциация (ДВП)"]
    K --> L[Усиление синаптических связей]
    K --> M[Корреляция с Φₑ]
    J --> N[Рекуррентность и пластичность]
    N --> O[Интеграция информации в контурах]
    N --> P[Активизация внутриклеточных каскадов]
    P --> Q[Экспрессия генов и синтез белков]
    P --> R[Структурные изменения синапсов]

    A --> V[Математическая модель]
    V --> W["M(t) = M₀ * exp(-t/τ) + ∫K(s)Φₑ(t-s)ds"]
    W --> X[Сила памятного следа]
