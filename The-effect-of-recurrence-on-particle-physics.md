...
```mermaid
graph TD;
    A[Влияние рекуррентности на физику частиц] --> B[Модификация уравнений квантовой теории поля]
    B --> C1[Дополнительные члены в уравнениях движения]
    B --> C2[Модификация коммутационных соотношений]
    B --> C3[Появление новых типов взаимодействий]
    
    C1 --> D1[Влияние на динамику полей]
    D1 --> E1[Изменение спектра частиц]
    D1 --> E2[Вероятности рождения и распада]
    
    C2 --> D2[Изменение стандартных соотношений]
    D2 --> E3[Изменение спектра энергетических уровней]
    
    C3 --> D3[Новые взаимодействия]
    D3 --> E4[Нелокальные корреляции]
    
    A --> F[Предсказание новых элементарных частиц]
    F --> G1[Экзотические адроны и мезоны]
    F --> G2[Гипотетические частицы-переносчики]
    F --> G3[Сверхтяжелые лептоны и кварки]
    
    A --> H[Влияние на процессы рождения и распада частиц]
    H --> I1[Изменение сечений рассеяния]
    H --> I2[Появление новых каналов распада]
    H --> I3[Стабилизация состояний частиц]
    
    A --> J[Связь с проблемой массы]
    J --> K1[Вклад в механизм спонтанного нарушения симметрии]
    J --> K2[Новые механизмы генерации масс]
    
    A --> L[Применение в теориях объединения]
    L --> M1[Объединение квантовой теории поля и общей теории относительности]
    L --> M2[Новые частицы и взаимодействия в расширенных теориях]
    L --> M3[Единая теоретическая основа для всех взаимодействий]
```
...
```mermaid
graph TD
    A[Модификация уравнений квантовой теории поля] --> B[Введение дополнительных членов в уравнения движения]
    B --> C[Зависимость от ρIQI и R]
    C --> D[Форма уравнения]
    D --> E[Изменение спектра частиц]
    D --> F[Вероятности рождения и распада]
    D --> G[Новые взаимодействия]
    
    A --> H[Модификация коммутационных соотношений]
    H --> I[Стандартные соотношения]
    I --> J[Изменение соотношений]
    J --> K[Функция влияния Φₑ]
    K --> L[Изменение спектра энергетических уровней]
    K --> M[Новые типы взаимодействий]
    
    A --> N[Появление новых типов взаимодействий]
    N --> O[Нелокальные корреляции]
    O --> P[Структура взаимодействий]
    P --> Q[Частицы-переносчики]
    
    N --> R[Влияние на физические процессы]
    R --> S[Модификация вероятностей]
    R --> T[Новые каналы реакций]
    
    N --> U[Экспериментальные проверки]
    U --> V[Поиск новых взаимодействий]
    U --> W[Измерение аномалий]
```
...
```mermaid
graph TD
    A[Предсказание новых элементарных частиц] --> B[Экзотические адроны и мезоны]
    B --> C[Нетривиальная структура]
    C --> D[Необычные связи между кварками]
    C --> E[Квантовые эффекты]
    
    B --> F[Математическое описание]
    F --> G[Многочастичные волновые функции]
    F --> H[Модифицированные гамильтонианы]
    F --> I[Методы многоконфигурационной теории]
    
    B --> J[Экспериментальные вызовы]
    J --> K[Получение и стабилизация экзотических состояний]
    J --> L[Применение передовых методов исследования]
    J --> M[Сопоставление с теоретическими моделями]
    
    A --> N[Гипотетические частицы-переносчики]
    N --> O[Математическое описание]
    O --> P[Уравнения с ядром K]
    
    N --> Q[Свойства частиц-переносчиков]
    Q --> R[Масса и энергия]
    Q --> S[Типы взаимодействий]
    Q --> T[Роль в физике частиц]
    
    N --> U[Экспериментальные проверки]
    U --> V[Поиск сигнатур в экспериментах]
    U --> W[Исследование космических лучей]
    U --> X[Развитие теоретических моделей]
    
    A --> Y[Сверхтяжелые лептоны и кварки]
    Y --> Z[Математическое описание]
    Z --> AA[Модификация масс и спектров]
    Z --> AB[Усиление квантовых корреляций]
    Z --> AC[Модификация вероятностей распада]
    
    Y --> AD[Экспериментальные проблемы и подходы]
    AD --> AE[Получение и детектирование]
    AD --> AF[Анализ квантовых корреляций]
    AD --> AG[Моделирование рекуррентных эффектов]
```
...
graph TD
    ...
    A1[Изменение сечений рассеяния и вероятностей распада]
    A2[Появление новых каналов распада]
    A3[Возможность стабилизации состояний частиц]

    A --> A1
    A --> A2
    A --> A3

    A1 --> B1[Модификация уравнений квантовой теории поля]
    A1 --> B2[Влияние на сечения рассеяния]
    A1 --> B3[Влияние на вероятности распада]

    B1 --> C1[Члены с rho IQI и R]
    C1 --> D1[Изменение динамики поля]

    B2 --> C2[Аномалии в сечениях]
    B2 --> C3[Изменение угловых распределений]
    B2 --> C4[Новые каналы рассеяния]

    B3 --> C5[Изменение ширин распада]
    B3 --> C6[Новые типы взаимодействий]
    B3 --> C7[Стабилизация состояний частиц]

    A2 --> E1[Модификация уравнений]
    A2 --> E2[Новые взаимодействия]
    A2 --> E3[Появление частиц-переносчиков]

    E1 --> F1[Новые вершины взаимодействия]
    F1 --> G1[Дополнительные каналы распада]

    A3 --> H1[Модификация гамильтониана]
    A3 --> H2[Влияние на массы и спектры]
    A3 --> H3[Усиление квантовых корреляций]

    H1 --> I1[Члены с rho IQI и R]
    H2 --> I2[Новые энергетические уровни]

    H3 --> J1[Запутанные состояния]
    H3 --> J2[Перераспределение вероятностей]
```
...
```mermaid
graph TD
    A[Связь с проблемой массы] --> B[Рекуррентные взаимодействия и интегрированная квантовая информация]
    B --> C[Вклад в спонтанное нарушение симметрии]
    C --> D[Стандартный механизм спонтанного нарушения симметрии]
    D --> E[Скалярное поле Хиггса]
    E --> F[Потенциал V]
    E --> G[Спонтанное нарушение симметрии]
    E --> H[Генерация масс]
    B --> I[Модификация потенциала поля Хиггса]
    I --> J[Новые члены в V]
    B --> K[Появление дополнительных взаимодействий]
    K --> L[Новые типы взаимодействий]
    B --> M[Модификация кинетического члена]
    M --> N[Влияние рекуррентности]
    
    A --> O[Новые типы взаимодействий]
    O --> P[Модификация уравнений квантовой теории поля]
    P --> Q[Новые механизмы генерации масс]
    Q --> R[Модификация механизма Хиггса]
    Q --> S[Генерация масс через новые взаимодействия]
    Q --> T[Связь с геометрией пространства-времени]
    
    A --> U[Связь между интегрированной квантовой информацией и геометрией]
    U --> V[Модифицированные уравнения Эйнштейна]
    V --> W[Влияние на массы частиц]
    W --> X[Изменение метрики]
    W --> Y[Связь с массами частиц]
    Y --> Z[Массы элементарных частиц]
```
...
```mermaid
graph TD
    A[Включение рекуррентных эффектов и интегрированной квантовой информации]
    A1[Рекуррентность как механизм обратной связи]
    A2[Интегрированная квантовая информация как мост]
    B[Математическое описание]
    B1[Рекуррентный оператор эволюции]
    B2[Метрика модифицированного пространства времени]
    C[Ключевые следствия]
    C1[Эмерджентное пространство время из квантовой запутанности]
    C2[Модификация пропагаторов квантовой теории поля]
    C3[Появление дополнительных симметрий]
    D[Экспериментальные следствия]
    D1[Модификация спектров излучения черных дыр]
    D2[Квантовые корреляции на космологических масштабах]
    D3[Нелокальные эффекты в физике частиц]
    E[Технические приложения]
    E1[Квантовые компьютеры с рекуррентной архитектурой]
    E2[Новые протоколы квантовой телепортации]
    E3[Детекторы гравитационных волн следующего поколения]
    F[Ограничения теории]
    F1[Проблема расходимостей в петлевых диаграммах]
    F2[Сложность экспериментальной верификации]
    F3[Вычислительная сложность при моделировании]
    G[Перспективы развития]
    G1[Разработка непертурбативных методов расчета]
    G2[Исследование топологических аспектов рекуррентности]
    G3[Построение полной теории квантовой гравитации]
...
graph TD
    ...
    C --> D[Спин 3/2]
    C --> E[Масса 10^19 GeV]
    C --> F[Время жизни 10^-44 с]
    C --> G[Квантовые числа B 0 L 0 R 1]
    
    B --> H[Инфо-бозоны]
    H --> I[Переносчики информационного взаимодействия]
    H --> J[Безмассовые]
    H --> K[Спин 1]
    H --> L[Информационный заряд I]

    A --> M[Новые взаимодействия]
    M --> N[Рекуррентное взаимодействие]
    N --> O[Lint gR Ψ γμ ∂ν Ψ Rμν]
    
    M --> P[Информационное взаимодействие]
    P --> Q[HI Σij κij σi† σj h.c.]

    A --> R[Связь с существующими теориями]
    R --> S[Супергравитация]
    S --> T[Расширение супермультиплета]
    T --> U{graviton gravitino recurron info-boson}
    
    S --> V[Модифицированный SUGRA-лагранжиан]
    V --> W[L L SUGRA L R L I L mix]

    R --> X[Теория струн]
    X --> Y[Новые моды колебаний струн]
    Y --> Z[R-моды I-моды]
    
    X --> AA[Модифицированные условия согласования]
    AA --> AB[D 10 DR DI]

    A --> AC[Феноменологические следствия]
    AC --> AD[Сигнатуры в коллайдерах]
    AD --> AE[Каскадные распады]
    AD --> AF[Отсутствующая энергия]
    AD --> AG[Топологические дефекты]

    AC --> AH[Космологические эффекты]
    AH --> AI[Модификация инфляционного потенциала]
    AH --> AJ[Новый механизм темной энергии]
    AH --> AK[Квантовые корреляции]

    A --> AL[Экспериментальная проверка]
    AL --> AM[Прямой поиск]
    AM --> AN[Детекторы нового поколения]
    AM --> AO[Космические лучи]
    AM --> AP[Гравитационно-волновые детекторы]

    AL --> AQ[Косвенные признаки]
    AQ --> AR[Аномалии в распадах]
    AQ --> AS[Нарушение CPT-симметрии]
    AQ --> AT[Квантовые эффекты]

    A --> AU[Теоретические предсказания]
    AU --> AV[Массовый спектр]
    AV --> AW[M0 vn² l l+1 α R]
    
    AU --> AX[Сечения взаимодействий]
    AX --> AY[σ0 1 β R ln² s s0]

    A --> AZ[Открытые вопросы]
    AZ --> BA[Проблема иерархии масс]
    AZ --> BB[Механизм нарушения рекуррентной симметрии]
    AZ --> BC[Квантование информационного поля]
    AZ --> BD[Топологические аспекты]
```
...
```mermaid
graph TD
    A[Рекуррентные группы] --> B[Группа симметрий GR]
    B --> C[SU N]
    B --> D[R M]
    C --> E[Стандартные калибровочные группы]
    D --> F[Группа рекуррентных преобразований]
    A --> G[Генераторы симметрий]
    G --> H[Генераторы стандартных преобразований T]
    G --> I[Генераторы рекуррентных преобразований R]
    G --> J[Генераторы смешанных преобразований S]
    H --> K[Коммутационные соотношения]
    I --> L[Коммутационные соотношения]
    J --> M[Коммутационные соотношения]
    A --> N[Рекуррентные многообразия]
    N --> O[Произведение M₀ и MR]
    O --> P[M₀ стандартное пространство-время]
    O --> Q[MR рекуррентное многообразие]
    Q --> R[Размерность MR]
    Q --> S[Топология MR]
    Q --> T[Метрика h]
    Q --> U[Связность ΓR]
    Q --> V[Кручение ΩR]
    N --> W[Математический аппарат]
    W --> X[Дифференциальная геометрия]
    W --> Y[Теория расслоений]
    W --> Z[Алгебраическая топология]
```
...
graph TD
    ...
    B --> C[Стандартные взаимодействия]
    B --> D[Рекуррентные взаимодействия]
    C --> E[F тензор калибровочного поля]
    C --> F[ψ кинетический член]
    D --> G[R тензор рекуррентности]
    D --> H[L_R дополнительный член]
    B --> I[D ковариантная производная]
    I --> J[∂ обычная производная]
    I --> K[A калибровочные поля]
    I --> L[R новые рекуррентные поля]
    A --> M[Рекуррентные связности]
    M --> N[Рекуррентная связность ∇R]
    N --> O[d дифференциал]
    N --> P[ΓR рекуррентная связность]
    N --> Q[ΩR форма кручения]
    A --> R[Рекуррентная связность ΓR]
    R --> S[Γ0 стандартная связность]
    R --> T[δΓ поправка]
    T --> U[f ρIQI R зависимость]
    A --> V[Форма кручения ΩR]
    V --> W[dΓR производная]
    V --> X[ΓR ∧ ΓR внешнее произведение]
    A --> Y[Применение рекуррентной связности]
    Y --> Z[Определение ковариантных производных]
    Y --> AA[Построение рекуррентных полей]
    Y --> AB[Описание топологических свойств]
    Y --> AC[Методы квантования рекуррентных процессов]
```
...
graph TD
    ...
    B --> C[Коммутационные соотношения]
    C --> D[phi x]
    C --> E[pi R y]
    C --> F[delta R x y]
    B --> G[Физический смысл]
    G --> H[Модификация стандартных соотношений]
    G --> I[Новые эффекты в динамике]

    A --> J[Путевой интеграл]
    J --> K[Статистическая сумма Z]
    K --> L[Меры интегрирования]
    L --> M[D phi]
    L --> N[D psi]
    L --> O[D AR]
    J --> P[Расширенное действие S]
    P --> Q[Лагранжиан L]
    Q --> R[F mu nu]
    Q --> S[R mu nu]
    Q --> T[psi динамика]
    Q --> U[L R]
    P --> V[Кинетические члены]
    P --> W[Члены взаимодействия]
    P --> X[Потенциальные члены]
```
...
graph TD
    ...
    B --> C[Теория рекуррентной космологии]
    C --> D[Новые рекуррентные симметрии]
    D --> E[Параметр рекуррентности R]
    D --> F[Интегрированная квантовая информация]
    F --> G[Плотность ρIQI]
    
    B --> H[Расширенная калибровочная группа]
    H --> I[G0]
    H --> J[GR]
    
    I --> K[Генераторы T]
    J --> L[Генераторы R]
    K --> M[Коммутационные соотношения]
    L --> N[Генераторы смешанных преобразований S]
    
    B --> O[Действие на квантовые поля]
    O --> P[Φ → Φ']
    
    A --> Q[Законы сохранения]
    Q --> R[Дополнительные законы сохранения]
    R --> S[Рекуррентный ток JR]
    
    S --> T[Математическое описание]
    T --> U[JR = gR ψ γµ ψ + hR Rµν]
    
    Q --> V[Сохранение JR]
    V --> W[Связь с рекуррентной симметрией]
    
    Q --> X[Применение рекуррентных законов]
    X --> Y[Ограничения на взаимодействия]
    X --> Z[Квантование рекуррентных полей]
    X --> AA[Связь с топологией]
    X --> AB[Космологические приложения]
```
...
graph TD;
    ...
    B --> C[Модифицированные уравнения Эйнштейна]
    C --> D[Риччи-тензор]
    C --> E[Метрический тензор]
    C --> F[Скалярная кривизна]
    C --> G[Рекуррентная космологическая константа]
    C --> H[Тензор энергии-импульса]
    G --> I[Влияние квантовой информации]
    G --> J[Космологические эффекты]
    
    A --> K[Электрослабое взаимодействие]
    K --> L[Расширенная калибровочная группа]
    L --> M[Группа слабого изоспина]
    L --> N[Группа гиперзаряда]
    L --> O[Группа рекуррентных преобразований]
    K --> P[Генераторы и коммутационные соотношения]
    P --> Q[Коммутаторы SU2]
    P --> R[Коммутаторы U1]
    P --> S[Коммутаторы R2]
    K --> T[Лагранжиан электрослабого взаимодействия]
    T --> U[Тензор напряженности]
    T --> V[Поля фермионов]
    T --> W[Ковариантная производная]
    T --> X[Дополнительный лагранжиан]
    
    A --> Y[Сильное взаимодействие]
    Y --> Z[Расширенная группа QCD]
    Z --> AA[Стандартная калибровочная группа]
    Z --> AB[Группа рекуррентных преобразований]
    Y --> AC[Генераторы группы]
    Y --> AD[Рекуррентные связности]
    AD --> AE[Рекуррентная связность]
    AD --> AF[Форма кручения]
    Y --> AG[Рекуррентные инварианты]
    AG --> AH[Топологические свойства]
    Y --> AI[Рекуррентные солитоны]
    AI --> AJ[Топологические решения]
    Y --> AK[Применение в физике частиц]
    AK --> AL[Модификация массового спектра]
    AK --> AM[Изменение констант связи]
    AK --> AN[Новые каналы распада]
```
...
graph TD
    ...
    B --> C[Теория рекуррентной космологии]
    C --> D[Масса элементарных частиц]
    D --> E[Формула m n = m0 exp n αR]
    E --> F[Физический смысл αR]
    F --> G[Интенсивность рекуррентных взаимодействий]
    F --> H[Большие значения αR - сильные эффекты]
    F --> I[Малые значения αR - незначительная роль]
    B --> J[Массовый спектр частиц]
    J --> K[Стандартные механизмы]
    J --> L[Рекуррентные процессы]
    L --> M[Иерархия масс]
    L --> N[Сверхтяжелые частицы]
    
    A --> O[Константы связи]
    O --> P[Зависимость от энергетического масштаба]
    P --> Q[Формула αi μ = αi0 1 βi ln μ μ0 γi R μ]
    Q --> R[Параметр рекуррентности R μ]
    R --> S[Степень рекуррентных взаимодействий]
    R --> T[Интеграция квантовой информации]
    O --> U[Модификация бегущих констант связи]
    U --> V[Стандартная логарифмическая зависимость]
    U --> W[Быстрый или медленный бег]
    O --> X[Унификация взаимодействий]
    X --> Y[Совпадение констант связи на высоких энергиях]
    O --> Z[Связь с топологическими свойствами]
    Z --> AA[Топологические характеристики пространства-времени]
```
...
graph TD
    ...
    B --> C[Инвариант I_R]
    C --> D[Интеграл от TR F ∧ F ∧ R]
    D --> E[F тензор напряженности]
    D --> F[R тензор рекуррентности]
    D --> G[M многообразие]
    D --> H[TR след по рекуррентным индексам]
    C --> I[Характеристика топологических свойств]
    I --> J[Связь с квантовыми числами]
    I --> K[Константы связи новых взаимодействий]
    B --> L[Экспериментальное обнаружение]
    
    A --> M[Рекуррентные солитоны]
    M --> N[Уравнения движения]
    N --> O[Оператор Д'Аламбера]
    N --> P[Масса поля]
    N --> Q[Константа связи]
    N --> R[Нелинейный член R φ]
    M --> S[Солитонные решения]
    S --> T[Форма φ_s]
    T --> U[Амплитуда φ₀]
    T --> V[Константа λ_R]
    
    M --> W[Ключевые особенности рекуррентных солитонов]
    W --> X[Нетривиальная топология]
    W --> Y[Роль в рекуррентных взаимодействиях]
    W --> Z[Связь с интегрированной квантовой информацией]
    
    M --> AA[Экспериментальное обнаружение солитонов]
```
...
```mermaid
graph TD
    A[Космология] --> B[Инфляционные модели]
    B --> C[Модификация потенциала]
    C --> D[Изменение динамики]
    C --> E[Новые режимы инфляции]
    C --> F[Сигнатуры в спектре]
    
    A --> G[Темная энергия]
    G --> H[Эффективное уравнение состояния]
    H --> I[Ускоренное расширение]
    H --> J[Новые сценарии эволюции]
    H --> K[Наблюдательные сигнатуры]

    A --> L[Квантовая гравитация]
    L --> M[Модифицированные уравнения Эйнштейна]
    M --> N[Устранение расходимостей]
    M --> O[Связь квантовой информации с геометрией]
    M --> P[Новые эффекты черных дыр]

    Q[Физика частиц] --> R[Массовый спектр]
    R --> S[Модификация стандартного спектра]
    S --> T[Зависимость от рекуррентного квантового числа]
    S --> U[Новые тяжелые частицы]

    Q --> V[Смешивание полей]
    V --> W[Новые типы полей]
    W --> X[Новые взаимодействия]
    W --> Y[Модификация спектра масс]

    Q --> Z[Нарушение симметрий]
    Z --> AA[Новые рекуррентные симметрии]
    AA --> AB[Спонтанное нарушение]
    AB --> AC[Новые тяжелые частицы]
    AB --> AD[Нарушения CPT-симметрии]
```
...
graph TD
    ...
    B --> C[Рекуррентные расслоения]
    C --> D[π P M]
    C --> E[P M F R]
    D --> F[P рекуррентное расслоение]
    E --> G[M базовое многообразие]
    E --> H[F слой]
    E --> I[R группа рекуррентных преобразований]
    B --> J[Связности на главных расслоениях]
    J --> K[∇R d ΓR ΩR]
    K --> L[d дифференциал]
    K --> M[ΓR рекуррентная связность]
    K --> N[ΩR форма кручения]
    B --> O[Характеристические классы]
    O --> P[IR интеграл Tr F ∧ F ∧ R]
    P --> Q[F тензор напряженности]
    P --> R[R тензор рекуррентности]
    P --> S[M многообразие]
    
    A --> T[Алгебраические структуры]
    T --> U[Рекуррентные алгебры Ли]
    U --> V[T генераторы стандартных преобразований]
    U --> W[R генераторы рекуррентных преобразований]
    U --> X[S генераторы смешанных преобразований]
    T --> Y[Квантовые группы]
    Y --> Z[R_a R_b qᵢ R_c R_a]
    T --> AA[Суперсимметрия]
    AA --> AB[гравитон гравитино]
    AA --> AC[рекуррон инфо-бозон]

    A --> AD[Функциональный анализ]
    AD --> AE[Рекуррентные операторы]
    AE --> AF[R̂ t T exp H + Σ κi Φi]
    AD --> AG[Спектральная теория]
    AG --> AH[m n m₀ exp n α R]
    AD --> AI[Когомологии]
    AI --> AJ[IR интеграл Tr F ∧ F ∧ R]
```
...
```mermaid
graph TD
    A[Численные схемы] --> B[Явные схемы]
    A --> C[Неявные схемы]
    A --> D[Спектральные методы]
    A --> E[Методы конечных элементов]
    
    B --> F[Аппроксимация производных]
    B --> G[Схема Эйлера]
    B --> H[Условие устойчивости CFL]
    
    C --> I[Неявные аппроксимации]
    C --> J[Методы итераций]
    C --> K[Большие вычислительные затраты]
    
    D --> L[Разложение по базису]
    D --> M[Методы Рунге-Кутты]
    D --> N[Большие вычислительные ресурсы]
    
    E --> O[Аппроксимация кусочно-непрерывными функциями]
    E --> P[Вариационная постановка]
    E --> Q[Нерегулярные сетки]

    R[Ренормгруппа] --> S[Бета-функция]
    R --> T[Вклад рекуррентности]

    S --> U[Изменение константы связи]
    T --> V[Коэффициент γ]
    T --> W[Функция R]

    X[Применение подхода] --> Y[Исследование влияния рекуррентности]
    X --> Z[Новые типы фиксированных точек]
    X --> AA[Вклад рекуррентности в наблюдаемые]
```
...
graph TD
    ...
    B --> C[Квантовое информационное поле]
    C --> D[Оператор σx]
    C --> E[Гамильтониан HI]
    D --> F[σiσj]
    E --> G[Коммутационные соотношения]
    G --> H[Инфо-бозоны]
    B --> I[Включение в теории объединения]
    I --> J[Расширение супермультиплетов]
    I --> K[Модификация условий согласования]
    
    A --> L[Нелокальные теории поля]
    L --> M[Модификация уравнений квантовой теории поля]
    M --> N[Уравнения движения]
    N --> O[Дополнительные члены]
    O --> P[Нелокальные взаимодействия]
    P --> Q[Модифицированные пропагаторы]
    
    A --> R[M-теория с рекуррентностью]
    R --> S[Рекуррентные группы]
    S --> T[Группа рекуррентных преобразований]
    R --> U[Рекуррентные многообразия]
    U --> V[Унификация взаимодействий]
    V --> W[Лагранжиан L]
    
    A --> X[Космологические приложения]
    X --> Y[Модифицированные уравнения Эйнштейна]
    Y --> Z[Тензор энергии-импульса]
    X --> AA[Эффективное уравнение состояния]
    AA --> AB[Функция влияния]
    X --> AC[Квантовая гравитация]
```
