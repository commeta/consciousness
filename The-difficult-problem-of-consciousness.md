...
```mermaid
graph TD
    A[Математическая формализация ЭИРО] --> B["Эмерджентная интегрированная информация (Φₑ)"]
    B --> C[Количественная мера сознания]
    C --> D[Модель Φₑ]
    D --> D1["Φₑ = ∫ I(integration) ⋅ R(recurrence) dt"]
    D1 --> E["Параметр I(integration)"]
    D1 --> F["Параметр R(recurrence)"]

    A --> G[Уравнения состояния нейронных сетей]
    G --> H[Динамика сети]
    H --> I[Модель состояния]
    I --> I1["dx/dt = f(x(t), u(t), W)"]
    I1 --> J[Рекуррентные связи]

    A --> K[Байесовское обновление моделей]
    K --> L[Предсказание и адаптация]
...
```mermaid
flowchart TD
    A[Иерархическая организация мозга] --> B[Первичные сенсорные зоны]
    A --> C[Вторичные ассоциативные области]
    A --> D[Медиальные структуры]
    B --> B1["Первичная зрительная кора (V1)"]
    B --> B2["Первичная слуховая кора (A1)"]
    B --> B3["Первичная соматосенсорная кора (S1)"]
    B1 --> B11[Ориентация, контраст, движение]
    B2 --> B12[Частота, интенсивность, временная структура]
    B3 --> B13[Давление, температура, положение тела]
    C --> C1["Вентральный зрительный поток (IT)"]
    C --> C2["Дорсальный зрительный поток (PPC)"]
    C --> C3["Префронтальная кора (PFC)"]
    C1 --> C11[Распознавание объектов]
    C1 --> C12[Категоризация и семантика]
    C1 --> C13[Иерархическая интеграция]
    C2 --> C21[Пространственная обработка]
    C2 --> C22[Контроль действий]
    C2 --> C23[Интеграция сенсомоторных взаимодействий]
    C3 --> C31[Исполнительный контроль]
    C3 --> C32[Модуляция внимания]
    C3 --> C33[Рабочая память]
    D --> D1[Гиппокамп]
    D --> D2[Парагиппокампальная кора]
    D1 --> D11[Кодирование эпизодической памяти]
    D1 --> D12[Интеграция сенсорной информации]
    D1 --> D13[Пространственно-временная контекстуализация]
    D2 --> D21[Кодирование пространственной информации]
    D2 --> D22[Обработка временной информации]
    D2 --> D23[Связывание пространства и времени]
    A --> E[Рекуррентные нейронные сети]
    E --> E1[Рекуррентные связи]
    E --> E2[Циклические потоки информации]
    E --> E3[Интеграция информации]
    A --> F[Синаптическая пластичность и обучение]
    F --> F1["Долговременная потенциация (LTP)"]
    F --> F2[Гомеостатическая пластичность]
    F1 --> F11[Активация NMDA-рецепторов]
    F1 --> F12[Фосфорилирование и экзоцитоз AMPA-рецепторов]
    F1 --> F13[Структурные изменения синапса]
    F2 --> F21[Масштабирование синапсов]
    F2 --> F22[Регуляция порога возбуждения]
...
graph TD
    ...
    B --> C["Эмерджентная интегрированная информация (Φₑ)"]
    C --> D[Математическая модель Φₑ]
    D --> D1["Φₑ = ∫ I(integration) ⋅ R(recurrence) dt"]

    A --> E[Интеграция информации]
    E --> F[Объединение сенсорных, моторных и когнитивных компонентов]
    F --> G["Параметр I(integration)"]
    G --> G1["I(integration) = H(total) - H(joint)"]
    G1 --> H["H(total) = Σ H(Xi)"]
    G1 --> I["H(joint) = H(X1, X2, ..., Xn)"]

    A --> J[Рекуррентность]
    J --> K[Циркуляция информации в сети]
    K --> L["Параметр R(recurrence)"]
    L --> M[Модель динамики сети]
    M --> M1["dx/dt = f(x(t), u(t), W)"]

    A --> N[Предсказательное кодирование]
    N --> O[Иерархическая байесовская система]
    O --> P[Байесовское обновление моделей]
    P --> P1["P(θ|D) = (P(D|θ) ⋅ P(θ)) / P(D)"]

    A --> Q[Интеграция теории и экспериментов]
    Q --> R["Нейровизуализация (фМРТ, ЭЭГ, МЭГ)"]
    Q --> S[Электрофизиология]
    Q --> T[Оптогенетика]
