...
3. **Вычисление степени рекуррентности:**
   ...
   - Используйте показатели, такие как **спектральный радиус** матрицы рекуррентных весов или другие динамические характеристики, чтобы оценить $R_{\text{recurrence}}(t)$.
...
#### Шаг 5: Вычисление произведения $I_{\text{integration}}(t) \times R_{\text{recurrence}}(t)$
...
- **Дискретный случай:**
  ...
  $$\Phi_e \approx \sum_{k=1}^{K} I_{\text{integration}}(t_k) \times R_{\text{recurrence}}(t_k) \times \Delta t$$
...
3. **Вычисление $\Phi_e$:**
   ...
   - $$\Phi_e = \sum_{k=1}^{10} (I_{\text{integration}}(t_k) \times R_{\text{recurrence}}(t_k) \times \Delta t)$$
