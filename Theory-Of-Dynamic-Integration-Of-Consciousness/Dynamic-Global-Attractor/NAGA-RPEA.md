...
flowchart LR
    Neu("[Нейронная сеть]")
    Ast("[Астроцитарная сеть]")
    Eph("[Эфаптическое поле]")

    Neu -->|Синаптическая передача| Ast
    Ast -->|ATP, глутамат, ГАМК, D-серин| Eph
    Eph -->|LFP, DC потенциалы| Neu
```
...
```mermaid
flowchart LR
    %% Узлы для расчёта W_eff
    Wb["W_baseline"]
    Ca["Ca_ast(t)"]
    ATP["ATP_ast(t)"]
    Glu["glutamate_spillover(t)"]
    LTP["LTP_LTD_factor(t)"]
    Weff["W_eff(t) = W_baseline × (1 + α·Ca_ast) × (1 + β·ATP_ast) × (1 + γ·glutamate_spillover) × LTP_LTD_factor"]
    ...
    %% Узлы для детектора корреляций
    Pre["pre_synaptic_j"]
    Post["post_synaptic_k"]
    CaResp["Ca_response_i(correlation_strength)"]
    Window["integration over sliding_window"]
    Corr["correlation_detection_i = ∫[window] (pre_synaptic_j × post_synaptic_k) × Ca_response_i dt"]
