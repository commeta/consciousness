...
```mermaid
graph TD
    A[Закон сохранения энергии и импульса] --> B[Классическая механика]
    A --> C[Общая теория относительности]
    A --> D[ЭИРО и интегрированная информация]
    
    B --> E[Теорема Нётер]
    E --> F[Однородность времени → Сохранение энергии]
    E --> G[Однородность пространства → Сохранение импульса]

    C --> H[Тензор энергии-импульса]
    H --> I[∇ᵤ Tᵤⁿ = 0]

    D --> J[Интегрированная квантовая информация]
    J --> K[Меры информации: энтропия, взаимная информация]
    D --> L[Рекуррентные процессы]
    L --> M[Обратные связи в квантовых системах]

    D --> N[Эмерджентные свойства]
    N --> O[Гравитация как эмерджентное явление]

    C --> P[Модификация тензора энергии-импульса]
    P --> Q[Дополнительный член Tⁿⁿ IQI]
    Q --> R[Плотность ρᵢᵩᵢ и давление pᵢᵩᵢ]
    Q --> S[Ковариантное сохранение ∇ᵤ Tⁿⁿ IQI = 0]

    C --> T[Новая модель Вселенной]
    T --> U[Эффекты, подобные темной энергии]
    T --> V[Соответствие наблюдениям: сверхновые, реликтовое излучение]

    A --> W[Согласование с теоремой Нётер]
    W --> X[Дополнительные симметрии → Обобщение законов сохранения]

    T --> Y[Экспериментальная проверка]
    Y --> Z[Гравитационные измерения и космология]
```
