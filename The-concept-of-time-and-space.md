...
```mermaid
graph TD
    A[Концепция времени и пространства] --> C[Теоретические основы]
    A --> D[Введение]
    A --> E[Применение в космологии]

    C --> F[Эмергентная интеграция информации]
    C --> G[Рекуррентные взаимодействия]
    F --> H[Параметры Φₑ и I инт]
    G --> I[Роль интеграции информации]

    D --> J[Вопросы структуры времени и пространства]
    D --> K[Рекуррентные процессы в квантовых системах]

    E --> L[Дискретность пространства-времени]
    E --> M[Дополнительные измерения]
    E --> N[Информационные параметры и их влияние]

    H --> O[Количественные оценки интеграции]
    K --> P[Связь с динамикой Вселенной]
    M --> Q[Математические доказательства]
    N --> R[Корреляции с наблюдениями]

    L --> S[Фрактальные структуры пространства]
    S --> T[Применение к современной космологии]
    T --> U[Подтверждение теории наблюдениями]
```
